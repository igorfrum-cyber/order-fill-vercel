...
Внутренний stateless gRPC-сервис расчетов заказа. Он выполняет ABC-классификацию, рассчитывает рекомендуемое и скорректированное количество, строит северный план распределения, планирует заказ до суммы и проверяет ручные правки. Сервис принимает уже структурированные строки и не читает Excel, не определяет бренд и не хранит результаты. Браузер бюджет не считает.
...
- `PlanBudget` — чистый пересчёт количеств до суммы: копейки, этапы C→B→A→A+, потолок 6 месяцев, пол месяца + поставка вниз. Не мутирует вход. Это не чтение Excel: вызывающая сторона шлёт сырые числа отчёта (`base_price`, `box_size`); скидку и брендовые кратности применяет сам сервис.
- `PlanBudget` для CHRISTINA PROFF (`group="proff"` + `line`) добавляет комплектный слой поверх обычного добора, повторяя `origin/main`. Скидка комплекта — 5% на каждую единицу полного набора из трёх (`ProcurementTotalCents` вычитает `savingCents`). Оракул стоимости шага задаёт компания (`ChristinaProffMode`): `standard` (пустое/неизвестное — то же) считает полный заказ; `fast` — инкремент O(s) только по затронутой линии, без второго полного total на горячем пути; `compare` гоняет **два независимых** прогона. Прикладные количества/сумма/комментарии всегда от `standard`; результат `fast` лежит в `FastRows`/`FastTotal`/`FastComplete`/`FastReason`/`FastLineSteps`, чтобы скачать JSON и увидеть расхождения. `compare` сам qty не меняет.
- Предпроход `CompleteChristinaLines` атомарно добирает наборы: ранжирование A+→A→B, затем отношение экономии к добавке; пороги — рост линии ≤130% базы (база фиксируется на старте, между шагами не сбрасывается), прирост скидки ≥15% добавки, итог ≤ цель×1,05. Исключение: первые три комплекта при одном отсутствующем артикуле добираются даже без продаж и не повторяются. Замки/исключения строк не обходятся.
...
internal/service/calculation       рекомендации, округление, north plan, бюджет, правки
...
| `PlanBudget` | `/orderfill.calculation.v1.CalculationService/PlanBudget` | Пересчитывает количества до суммы без мутации входа: скидка, брендовые кратности и комплект CHRISTINA PROFF (`group`/`line`); `reason` — `overSix`/`belowOne` или текст остановки. |
...
Основной клиент долгих книг — `document-worker`: он передает извлеченные из книг строки, правила бренда, потребности северных городов и получает рассчитанные значения для Excel и отчета. `gateway-service` вызывает только `PlanBudget` с `POST /api/v1/order/budget-plan`. Правила бренда сервис напрямую не запрашивает; их получает вызывающая сторона от `brand-service` и передает в `CalculateAdjustedQuantity`. Бюджет и перемещение 25% тоже считает этот сервис по уже извлечённым числам — Excel и HTTP сюда не входят.
...
Тесты покрывают рекомендации, брендовые коэффициенты, округление, north plan, PlanBudget и комплект CHRISTINA PROFF, ручные правки, gRPC mapping, defaults конфигурации и health handler.
...
- `PlanBudget` не сохраняет результат: это unary preview. Комплектная скидка CHRISTINA PROFF срабатывает только при `group="proff"` и заполненном `line`; `price`/`unit`/`step`/`minimum`/`delivery` на входе игнорируются — сервис считает от `base_price`, скидки и брендовых правил.
