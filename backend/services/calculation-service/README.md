...
- `PlanBudget` для CHRISTINA PROFF (`group="proff"` + `line`) добавляет комплектный слой поверх обычного добора, повторяя `origin/main`. Скидка комплекта — 5% на каждую единицу полного набора из трёх (`ProcurementTotalCents` вычитает `savingCents`, `changeCost` для PROFF пересчитывает весь заказ). Предпроход `CompleteChristinaLines` атомарно добирает наборы: ранжирование A+→A→B, затем отношение экономии к добавке; пороги — рост линии ≤130% базы (база фиксируется на старте, между шагами не сбрасывается), прирост скидки ≥15% добавки, итог ≤ цель×1,05. Исключение: первые три комплекта при одном отсутствующем артикуле добираются даже без продаж и не повторяются. Замки/исключения строк не обходятся.
