...
- Для `north_merge` требуется хотя бы один городской бланк. Город определяется сначала по содержимому книги, затем по имени файла: Сургут, Вартовск, Уренгой или Тюмень. Christina принимает раздельные роли `blank-home` и `blank-proff`; их позиции не смешиваются. Таблицы офиса Тюмени и склада доставки опциональны.
