...
- Для `order_fill` параллельно загружает исходную таблицу 1С, опциональную таблицу второго склада Тюмени и бланки. Перед расчётом второго склада складываются остатки, товары в пути и продажи одинаковых позиций; несовместимые периоды и дубли отклоняются. Затем сервис определяет месяц и бренд, рассчитывает рекомендации, сопоставляет позиции, заполняет книги и подставляет сохранённые реквизиты компании в распознанные поля ANGIOPHARM, Skin Synergy и KLAPP. Пустые настройки не затирают значения поставщика; отдельно заданная скидка поддерживает явные 0%. После этого сервис создаёт отчет и preview. Для строк с ABC-анализом отчет также передает средний спрос и цену бланка: закупочную, единственную скидочную либо единственную распознанную цену — эти данные используются экраном «Заказ до суммы». Стадия `process` завершает job в состоянии, предназначенном для review; `finalize` применяет ручные правки и завершает результат.
- Для `north_merge` извлекает потребности из городских бланков, опционально читает отдельные остатки офиса Тюмени и склада доставки, строит план через `calculation-service`, сохраняет review-отчет и на `finalize` повторно считает изменённые города на сервере. Если отдельного бланка Тюмени нет, её план берётся из `Заказано по факту`, а при пустом факте — из `Рекомендованного заказа` таблицы Тюмени. Итогом становятся заполненный общий бланк на вариант, отдельные перемещения по городам и preview каждого файла; отсутствующие в выбранном общем бланке заказанные позиции дописываются в конец.
...
- Для `north_merge` требуется хотя бы один городской бланк. Город определяется сначала по содержимому книги, затем по имени файла: Сургут, Вартовск, Уренгой или Тюмень. Christina принимает раздельные роли `blank-home` и `blank-proff`; их позиции не смешиваются. Строки бланка `blank-proff` получают принадлежность к линии CHRISTINA PROFF (id/name/article/required), которая доходит до северного отчёта (`christinaLine`), чтобы планировщик бюджета применил комплектную скидку на севере так же, как в Тюмени. Таблицы офиса Тюмени и склада доставки опциональны.
...
```json
{
  ...
  "order_profile": {
    ...
    "brand_terms": [{"brand": "angiopharm", "discount_basis_points": 3500, "discount_set": true}]
  },
  "inputs": [
...
Пакет содержит unit- и integration-style тесты: queue consumer, use cases, gRPC API, клиенты, распознавание колонок/периодов/брендов, заполнение и финальные правки, north report, preview, OOXML round-trip, формулы и стили. Private-fixture тест всех реальных бланков и таблиц продаж включается через `ORDER_FILL_PRIVATE_TESTDATA`; тяжелый large-file тест — через `ORDERFILL_BENCH`.
