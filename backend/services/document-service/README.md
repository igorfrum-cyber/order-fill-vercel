...
- Для `order_fill` параллельно загружает исходную таблицу 1С и бланк, определяет месяц и бренд, рассчитывает рекомендации, сопоставляет позиции, заполняет книги, создает отчет и preview. Стадия `process` завершает job в состоянии, предназначенном для review; `finalize` применяет ручные правки и завершает результат.
- Для `north_merge` извлекает потребности из городских бланков, опционально читает остатки Тюмени, строит план через `calculation-service`, сохраняет review-отчет и применяет правки на стадии `finalize`.
...
- Для обычного `order_fill` требуется одна исходная выгрузка с ролью `source` и ровно один бланк с ролью `blank`.
- Для `north_merge` требуется хотя бы один городской бланк. Город определяется по имени файла: Сургут, Вартовск, Уренгой или Тюмень. Исходная таблица Тюмени опциональна.
...
В окружении, отличном от `local` (без учета регистра), `ValidateAPI` требует `FILE_GRPC_ADDR` и `BRAND_GRPC_ADDR`, а `ValidateWorker` — все шесть внешних адресов/URL. В `local` ранняя проверка пропускается, но bootstrap воркера все равно создает реальные клиенты и consumer, поэтому пустые адреса не дают работоспособный standalone-режим.
...
- Некорректный payload подтверждается (`XACK`) после логирования. Ошибка handler также логируется, после чего текущее сообщение подтверждается; автоматического повторного выполнения на этой ошибке нет. Use case по возможности записывает failure в `job-service`.
