...
func (s *Store) GetSettings(ctx context.Context) (domain.Settings, error) {
	row := s.pool.QueryRow(ctx, "SELECT enabled, receive_address, last_webhook_at, webhook_count, error_count FROM inbound_settings LIMIT 1")
	var out domain.Settings
	var last sql.NullTime
	err := row.Scan(&out.Enabled, &out.ReceiveAddress, &last, &out.WebhookCount, &out.ErrorCount)
	if err != nil {
...
func (s *Store) UpsertSettings(ctx context.Context, enabled bool, receiveAddress string) (domain.Settings, error) {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO inbound_settings (enabled, receive_address) VALUES ($1, $2)
		ON CONFLICT (id) DO UPDATE SET enabled = $1, receive_address = $2
	`, enabled, receiveAddress)
	if err != nil {
...
func (s *Store) GetCompanyInbound(ctx context.Context, companyID string) (domain.CompanyInbound, error) {
	row := s.pool.QueryRow(ctx, "SELECT company_id, receive_address, allowed_from, sender_email, enabled FROM company_inbound WHERE company_id = $1", companyID)
	var out domain.CompanyInbound
	err := row.Scan(&out.CompanyID, &out.ReceiveAddress, &out.AllowedFrom, &out.SenderEmail, &out.Enabled)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
func (s *Store) GetCompanyByAllowedSender(ctx context.Context, senderEmail string) (domain.CompanyInbound, error) {
	addr := strings.TrimSpace(strings.ToLower(senderEmail))
	row := s.pool.QueryRow(ctx, "SELECT company_id, receive_address, allowed_from, sender_email, enabled FROM company_inbound WHERE LOWER(sender_email) = $1 AND enabled = TRUE", addr)
	var out domain.CompanyInbound
	err := row.Scan(&out.CompanyID, &out.ReceiveAddress, &out.AllowedFrom, &out.SenderEmail, &out.Enabled)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
		...
		return domain.CompanyInbound{}, fmt.Errorf("get company by allowed sender: %w", err)
	}
	return out, nil
}

func (s *Store) UpsertCompanyInbound(ctx context.Context, companyID, receiveAddress, senderEmail string, enabled bool) (domain.CompanyInbound, error) {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO company_inbound (company_id, receive_address, sender_email, enabled)
		VALUES ($1, $2, $3, $4)
...
func (s *Store) ListMessages(ctx context.Context, companyID string, limit int) ([]domain.MessageSummary, error) {
	...
	rows, err := s.pool.Query(ctx, `
		SELECT id, provider_message_id, subject, envelope_from, envelope_to, received_at, company_id, status, error_code, attachment_count, total_bytes
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.ProviderMessageID, &m.Subject, &m.EnvelopeFrom, &m.EnvelopeTo, &m.ReceivedAt, &m.CompanyID, &m.Status, &m.ErrorCode, &m.AttachmentCount, &m.TotalBytes); err != nil {
