...
// The DTOs below own the JSONB wire format of the jobs and job_reports columns.
// document-service writes job_reports.summary and job_reports.rows with exactly
// these snake_case names, so the tags are a cross-service contract and must not
// be renamed together with the domain fields.
...
func inputFilesToDTO(files []job.InputFile) []inputFileDTO {
	result := make([]inputFileDTO, 0, len(files))
	for _, file := range files {
		result = append(result, inputFileDTO{
			ID:          file.ID,
...
func inputFilesToDomain(files []inputFileDTO) []job.InputFile {
	result := make([]job.InputFile, 0, len(files))
	for _, file := range files {
		result = append(result, job.InputFile{
			ID:          file.ID,
			Role:        job.Role(file.Role),
			Name:        file.Name,
			ContentType: file.ContentType,
			SizeBytes:   file.SizeBytes,
			StorageKey:  file.StorageKey,
...
func outputFilesToDTO(files []job.OutputFile) []outputFileDTO {
...
func outputFilesToDomain(files []outputFileDTO) []job.OutputFile {
	result := make([]job.OutputFile, 0, len(files))
	for _, file := range files {
		result = append(result, job.OutputFile{
			ID:          file.ID,
			Label:       file.Label,
			Name:        file.Name,
			ContentType: file.ContentType,
...
func summaryToDTO(summary job.Summary) summaryDTO {
	return summaryDTO{
		Brand:                  summary.Brand,
...
func summaryToDomain(summary summaryDTO) job.Summary {
	return job.Summary{
		Brand:                  summary.Brand,
		OrderMonthLabel:        summary.OrderMonthLabel,
		AdjustmentLabel:        summary.AdjustmentLabel,
...
func reportRowsToDTO(rows []job.ReportRow) []reportRowDTO {
...
func reportRowsToDomain(rows []reportRowDTO) []job.ReportRow {
	result := make([]job.ReportRow, 0, len(rows))
	for _, row := range rows {
		result = append(result, job.ReportRow{
			Key:                 row.Key,
			Status:              row.Status,
			BlankID:             row.BlankID,
			BlankLabel:          row.BlankLabel,
...
func duplicateCandidatesToDTO(candidates []job.DuplicateCandidate) []duplicateCandidateDTO {
...
func duplicateCandidatesToDomain(candidates []duplicateCandidateDTO) []job.DuplicateCandidate {
	result := make([]job.DuplicateCandidate, 0, len(candidates))
	for _, candidate := range candidates {
		result = append(result, job.DuplicateCandidate{
			SourceRow:     candidate.SourceRow,
			SourceArticle: candidate.SourceArticle,
			SourceName:    candidate.SourceName,
			Recommended:   candidate.Recommended,
