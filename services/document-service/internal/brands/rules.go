...
func ArticleNormalizeOptions(rule RuleConfig) domain.ArticleOptions {
	return domain.ArticleOptions{PreserveHyphen: rule.PreserveArticleHyphen}
}

func CategoryCoefficient(value string, rule RuleConfig) float64 {
	category := domain.NormalizeCategory(value)
...
func CalculateAdjustedQuantity(recommended float64, rule RuleConfig, boxSizeValue string) AdjustedQuantity {
	rounded := domain.RoundHalfUp(recommended)
	...
	case AdjustmentMinimum:
		minimumValue, ok := domain.ParseNumber(boxSizeValue)
		...
		return AdjustedQuantity{Rounded: rounded, Inserted: floatPtr(float64(rounded))}
	default:
		boxSize, ok := domain.ParseNumber(boxSizeValue)
		if !ok || boxSize <= 0 {
			return AdjustedQuantity{Rounded: rounded, Inserted: floatPtr(float64(rounded))}
		}
