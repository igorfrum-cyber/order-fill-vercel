...
export function christinaLineGroups(rows) {
  ...
  for (const row of rows) {
    ...
    if (!groups.has(id)) groups.set(id, { id, name: row.line.name, rows: [], required: row.line.required, valid: true });
  ...
  for (const line of groups.values()) {
    ...
    line.valid &&= required.size > 0 && required.size === line.required.length
      && articles.length === required.size && new Set(articles).size === articles.length
...
export function procurementTotalCents(rows) {
  return rows.reduce((sum, r) => sum + toCents(r.price * r.quantity), 0)
    - christinaLineGroups(rows).reduce((sum, line) => sum + line.savingCents, 0);
}

export const LINE_COVERAGE = { C: 2, B: 2.5, A: 3, 'A+': 3.5 };
const months = (row, q) => row.demand > 0
  ? (row.stock + row.transit - (row.outbound || 0) + q * row.unit) / row.demand : null;
...
export function proposeLineStep(rows, id, baselines, target) {
  ...
  if (!firstSingle && changes.some(r => {
    const cover = months(r, nextSets);
    return cover == null || !Number.isFinite(r.delivery) || !LINE_COVERAGE[r.category]
      || cover > LINE_COVERAGE[r.category] + r.delivery + 1e-8 || cover > 6 + 1e-8;
  ...
  const addedCents = changes.reduce((sum, r) => sum + toCents(r.price * nextSets) - toCents(r.price * r.quantity), 0);
  const savedCents = after.savingCents - line.savingCents;
  const baseline = baselines.get(id);
  if (!(baseline > 0) || after.netCents > Math.floor(baseline * 1.3 + 1e-8)) return { accepted: false, reason: 'lineGrowth' };
  ...
  return { accepted: true, rows: proposed, id, name: line.name, sets: after.sets,
    addedCents, savedCents, totalCents, firstSingle,
...
export function completeChristinaLines(input, target, baselines = new Map(christinaLineGroups(input).map(l => [l.id, l.netCents]))) {
  ...
  for (let iteration = 0; iteration < 250000; iteration++) {
    const proposals = christinaLineGroups(rows).map(l => ({ id: l.id, ...proposeLineStep(rows, l.id, baselines, target) }));
    rejected = proposals.filter(p => !p.accepted).map(({ id, reason }) => ({ id, reason }));
    const candidates = proposals.filter(p => p.accepted);
    candidates.sort((a, b) => b.rank[0] - a.rank[0] || b.rank[1] - a.rank[1] || b.rank[2] - a.rank[2]
      || b.savedCents / b.addedCents - a.savedCents / a.addedCents || a.id.localeCompare(b.id));
    if (!candidates.length) return { rows, steps, rejected, baselines };
    const chosen = candidates[0];
    rows = chosen.rows;
    steps.push({ id: chosen.id, name: chosen.name, sets: chosen.sets, addedCents: chosen.addedCents, savedCents: chosen.savedCents });
  }
